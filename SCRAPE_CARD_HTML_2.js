    ...
    for (const link of linksData) {
        ...
        try {
            ...
            while (hasNextPage) {
                ...
                if (nextButton) {
                    await page.waitForSelector('tbody');
                 // await page.waitForSelector('#itemResults_paginate');
            ...
            // Save the HTML that has been captured so far
            const errorHtmlFilePath = path.join(folderPath, `error_${Date.now()}.html`);     // SOLVE THE NAMING!!
            ...
            const screenshotPath = path.join(folderPath, `error_${Date.now()}.png`);
