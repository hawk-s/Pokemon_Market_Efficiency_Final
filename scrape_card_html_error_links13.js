...
async function scrapeLinksAndSaveHTML(folderPath) {
    ...
    const linksJsonFilePath = path.join(folderPath, 'error_links13.json');
    ...
    if (errorLinks.length > 0) {
        const errorLinksFilePath = path.join(folderPath, 'error_links14.json');
...
async function processAllFolders(basePath) {
    ...
    for (const folderName of folderNames) {
        ...
        if (fs.existsSync(linksJsonFilePath)) {
            ...
            console.log(`No error_links13.json found in folder: ${folderPath}`);
        }
    }
}
