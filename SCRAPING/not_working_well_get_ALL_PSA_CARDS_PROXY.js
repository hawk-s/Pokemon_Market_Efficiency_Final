...
async function scrapeCardData(url, outputPath, proxy) {
    const browser = await puppeteer.launch({
        headless: false,
        args: [`--proxy-server=${proxy}`]  // Use the proxy server
    });
    const page = await browser.newPage();

    // Set user agent and other headers (optional)
    ...
    try {
        ...
        console.error(`Failed to scrape ${url} using proxy ${proxy}`, error);
    } finally {
        await browser.close();
    }
}
...
async function processSets() {
    ...
    const proxies = loadProxiesFromFile('proxies_list.txt');
    ...
    for (let i = 1; i <= 151; i++) {
        ...
        if (fs.existsSync(linksFilePath)) {
            ...
            for (let j = 0; j < links.length; j++) {
                ...
                console.log(`Processing ${url} -> ${outputPath} using proxy ${proxy}`);
                
                // Scrape the card data and save it to the appropriate set folder
                await scrapeCardData(url, outputPath, proxy);
            }
        } else {
