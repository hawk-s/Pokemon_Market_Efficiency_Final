// scrapes just one url of a set (here Unbroken Bonds)

const puppeteer = require('puppeteer');
const fs = require('fs');
const path = require('path');

async function downloadHtmlAfterTableLoad(singleUrl, outputFolder = 'TCG_PRICE_GUIDES_CARDS_FINAL', timeout = 20000) {
    try {
        ...
        try {
            ...
            // Navigate to the URL
            await page.goto(singleUrl, { waitUntil: 'networkidle2', timeout });
            ...
            const urlParts = singleUrl.split('/');
            ...
            console.log(`Downloaded HTML from ${singleUrl} and saved as ${outputFilename}`);

        } catch (error) {
            console.error(`Failed to process ${singleUrl}: ${error.name} - ${error.message}`);
        } finally {
            await page.close();
        }
...
// Example usage:
const specificUrl = "https://www.tcgplayer.com/categories/trading-and-collectible-card-games/pokemon/price-guides/sm-unbroken-bonds";
downloadHtmlAfterTableLoad(specificUrl);
